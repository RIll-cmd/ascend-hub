param(
    [string]$InstanceId = 'codex-local-1',
...
$settingsDirectory = Join-Path $env:LOCALAPPDATA "Ascend\status-adapters\$InstanceId"
...
if ((Test-Path -LiteralPath $configPath) -or (Test-Path -LiteralPath $credentialPath)) {
    throw 'Local Codex credential configuration already exists. No credential was created or rotated.'
}
...
try {
    ...
    # The existing CLI delegates secure creation/hash/binding/duplicate protection to StatusService.
    $provisionOutput = & $pythonPath -m server.cli.status_credentials create --service-id codex-cli --instance-id $InstanceId 2>&1
    if ($LASTEXITCODE -ne 0) { throw 'Core credential creation failed. No existing producer credential was changed.' }
    $plaintext = @($provisionOutput | Where-Object { $_ -is [string] -and $_ -match '^[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]{8,256}$' })
    if ($plaintext.Count -ne 1) { throw 'Credential creation returned an unexpected result. Check safe credential metadata before retrying.' }
    ...
    # Runtime operator secret storage, not an application log or repository file.
    ...
    Write-Host "Provisioned dedicated codex-cli/$InstanceId credential. Secret is DPAPI-protected for this Windows user."
